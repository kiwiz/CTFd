...
function updateChalWindow(obj) {
    $.get(script_root + '/api/v1/challenges/' + obj.id, (response) => {
        var challenge_data = response.data

        $.getScript(script_root + obj.script, () => {
            $.get(script_root + obj.template, (template_data) => {
                ...
                $('#challenge-window').on('hide.bs.modal', (e) => {
